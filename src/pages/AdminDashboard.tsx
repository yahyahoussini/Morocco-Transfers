...
import { Shield, Bell, X, Check, Clock, Phone, MapPin, Car, Download, HelpCircle, Smartphone, Route, Plus, Trash2, BarChart3, TrendingUp, DollarSign, CalendarIcon } from "lucide-react";
...
const AdminDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState<'bookings' | 'routes' | 'analytics'>('bookings');
  ...
  const sendSystemNotification = useCallback((booking: Booking) => {
    if ("Notification" in window && Notification.permission === "granted") {
      new Notification("New Booking! 🚕", {
        body: `${booking.passenger_name} — ${Number(booking.price).toLocaleString()} DH. Tap to view.`,
        tag: "booking-alert",
        icon: "/icon-192.png",
      });
    }
  ...
  const enableNotifications = async () => {
    ...
    if (permission === "granted") {
      ...
      new Notification("Notifications Active ✅", {
        body: "You will now receive alerts for new bookings.",
        icon: "/icon-192.png",
      });
    } else {
    ...
    <div className="min-h-screen bg-background px-4 py-6 pt-14 max-w-lg mx-auto">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setShowInstallHelp(true)} className="text-muted-foreground hover:text-foreground transition-colors" title="Install App">
        ...
        <>
          ...
          {!notificationsEnabled && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
              <Button onClick={enableNotifications} className="w-full gold-gradient text-primary-foreground font-semibold rounded-xl">
